...
2. Navigate to the DB directory where the docker-compose file locates.
   ...
   ```bash
   cd web3db-backend/DB/

   ```

3. Verify that the `docker-compose.yml` file exists in the root directory (web3db-backend/DB/). This file contains the configuration for all the necessary services.
...
<Admonition type="success">
  ...
  of web3db using Docker Compose. The Apache Spark, Apache
...
4. Access the Apache Hive NameNode UI by opening a web browser and navigating to `http://localhost:9870`. You should see the Hive DataNode UI.

5. Run a sample Spark job to ensure that Spark is functioning correctly. You can use the Spark shell or submit a Spark application.

6. Execute a sample Hive query to verify that Hive is working as expected. You can use the Hive CLI or connect to Hive using a Hive client.
